...
//}
  ...
  function audit($task) {
//      $this->Execute("insert into accountspecs (UserID,  lastSeen) " .
//          "                  values('{$_SESSION['UserID']}', now()) ");


        $this->Execute("update accountspecs set lastSeen=now() where UserID='".$_SESSION["UserID"]."'");


        $res = $this->Execute("select lastSeen from accountspecs where UserID='".$_SESSION["UserID"]."'");
        $result = $res->fetch();
