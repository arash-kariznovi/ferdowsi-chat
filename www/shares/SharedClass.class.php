...
class SharedClass
{
	...
	static function CreateSettingsThemeSelectOptions($RelatedTable, $RelatedValueField, $RelatedDescriptionField, $OrderBy = "")
	{
		if($OrderBy=="")
			$OrderBy = $RelatedValueField;
		$ret = "";
	...
	static function loadAndPerformTheme($mysql, $Person_ID){
		...
		FROM person_theme
		JOIN sadaf.persons
			ON person_theme.person_id = sadaf.persons.PersonID
		JOIN sadaf.theme
			ON person_theme.theme_id = sadaf.theme.theme_id
			where PersonID=?");
		$res1 = $mysql->ExecuteStatement(array($Person_ID));
		
		
		if($trec1 = $res1->fetch()){
			echo "
			<script>
				localStorage.setItem(\"color1\", \"" . strval($trec1["color1"]) . "\");
				localStorage.setItem(\"color2\", \"" . strval($trec1["color2"]) . "\");
				localStorage.setItem(\"color3\", \"" . strval($trec1["color3"]) . "\");
				localStorage.setItem(\"font\", \"" . strval($trec1["font"]) . "\");
				localStorage.setItem(\"font_size\", " . strval($trec1["font_size"]) . ");
				localStorage.setItem(\"status\", \"" . strval($trec1["status"]) . "\");
		...
		";
			...
			$mysql->Prepare("INSERT INTO sadaf.person_theme (person_id, theme_id, status) VALUES (?, 1, 'online')");
			$res1 = $mysql->ExecuteStatement(array($Person_ID));
			if($trec1 = $res1->fetch()){
				SharedClass::loadAndPerformTheme($mysql, $Person_ID);
			}
			else{
				// echo "<script>alert(\"khata daad\")</script>";
				SharedClass::loadAndPerformTheme($mysql, $Person_ID);
			}
